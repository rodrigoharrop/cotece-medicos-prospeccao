 ...
 * 21 Set → 02 Out 2026 | Turquia + Israel | 12 Dias / 11 Noites
...
export interface TabSlide {
  ...
  dayBadge: string;       // ex: "DIAS 2 & 3 • 22–23 SET"
  flag: string;
  country: string;        // "TURQUIA" | "ISRAEL" | ""
  title: string;          // grande — 2 linhas máx
  subtitle: string;       // sublinhado / destino
  description: string;
  highlights: string[];
  location: string;
  imageUrl: string;       // fundo principal
  imageUrl2?: string;     // fundo secundário (parallax direito)
  imageUrl2StartSec?: number; // fade-in da img2 neste segundo
  color: string;          // cor dominante do slide
  accentColor: string;
  icon: string;
  // narração (texto pronto para ElevenLabs — será habilitado depois)
  narration: string;
}

// Duração de cada slide em segundos (sem narração — baseado no conteúdo visual)
const DURATIONS_SEC = [18, 25, 20, 28, 18];
...
export const tabSlides: TabSlide[] = [
  // ── 0 — INTRO ────────────────────────────────────────────────────────────
  {
    ...
    highlights: [
      "12 Dias / 11 Noites",
      "Turquia & Israel",
      "Feat. dos Tabernáculos ICEJ",
    ],
    location: "São Paulo → Turquia → Israel",
    imageUrl: "tab/terra-santa-jerusalem.jpg",
    imageUrl2: "tab/caravana-tabernaculos-banner-v2.png",
    color: "#050a1a",
    accentColor: "#c9a84c",
    icon: "✨",
    narration:
  ...
  // ── 1 — TURQUIA: 7 IGREJAS ───────────────────────────────────────────────
  {
    ...
    description:
      "Pérgamo, Tiatira, Sardis, Filadélfia, Laodiceia, Esmirna, Éfeso — sete mensagens que ecoam para a Igreja de hoje.",
    highlights: [
      "Igreja Vermelha de Pérgamo — Bergama",
      "Sardis ao lado do Templo de Ártemis",
      "Laodiceia em Pamukkale — terraços brancos",
      "Éfeso — cidade de Paulo e João",
    ],
    location: "Pérgamo → Tiatira → Sardis → Filadélfia → Laodiceia → Izmir",
    ...
    icon: "🏛️",
    narration:
      "Na Turquia, percorremos os passos de João — o exilado de Patmos que viu o céu aberto. Pérgamo, Tiatira, Sardis, Filadélfia, Laodiceia. Sete igrejas. Sete mensagens. Um chamado que atravessa vinte séculos e ecoa diretamente para a Igreja de hoje.",
  },
  ...
  // ── 2 — GALILEIA & JORDÃO ────────────────────────────────────────────────
  {
    ...
    description:
      "Mar da Galileia, Cafarnaum, Monte das Bem-Aventuranças e batismo nas águas do Jordão — abertura da Festa dos Tabernáculos.",
    highlights: [
      "Barco no Mar da Galileia com louvor",
      "Cafarnaum — Sinagoga e Casa de Pedro",
      "Monte das Bem-Aventuranças & Tabgha",
      "Batismo em Yardenit — Rio Jordão",
    ],
    location: "Tiberíades → Yardenit → Deserto da Judeia → Jerusalém",
  ...
  // ── 3 — JERUSALÉM: FESTA DOS TABERNÁCULOS ────────────────────────────────
  {
    ...
    description:
      "Monte das Oliveiras, Getsêmani, Via Dolorosa, Jardim do Túmulo e a histórica Marcha de Jerusalém com cristãos do mundo inteiro.",
    highlights: [
      "Monte das Oliveiras & Getsêmani",
      "Via Dolorosa & Jardim do Túmulo (CITY)",
      "Marcha de Jerusalém — evento ICEJ",
      "Região da Festa Nova — Envelope de Gaza",
    ],
    location: "Jerusalém — Sucah, CITY, Eventos ICEJ",
    imageUrl: "tab/terra-santa-monte-oliveiras.jpg",
    imageUrl2: "tab/jerusalem-muro.jpg",
    imageUrl2StartSec: 14,
    color: "#1a0d00",
    accentColor: "#fbbf24",
    icon: "🕍",
    narration:
